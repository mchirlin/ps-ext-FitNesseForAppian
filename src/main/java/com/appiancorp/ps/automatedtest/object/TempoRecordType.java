    ...
    public static boolean waitFor(String type, Settings s) {
        try {
            if (isFieldIndex(type)) {
                int rNum = getIndexFromFieldIndex(type);
                String rName = getFieldFromFieldIndex(type);
                (new WebDriverWait(s.getDriver(), s.getTimeoutSeconds())).until(ExpectedConditions.presenceOfElementLocated(By.xpath(String.format(XPATH_ABSOLUTE_RECORD_TYPE_LINK_INDEX, rName, rNum))));
            }
            else{
                (new WebDriverWait(s.getDriver(), s.getTimeoutSeconds())).until(ExpectedConditions.presenceOfElementLocated(By.xpath(String.format(XPATH_ABSOLUTE_RECORD_TYPE_LINK, type))));
            }
            WebElement element = getRecordType(type, s);
